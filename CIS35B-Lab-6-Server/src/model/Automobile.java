...
public class Automobile implements Serializable {
	...
	public void updateOption(OptionSet o, String currName, String newName, float newPrice) throws AutoException {
		...
		try {
			...
			throw new AutoException(
					"Error with Thread, either the Optionset name does not exist or current option name does not exist");
		}
	...
	public ArrayList<String> getArrayListTable() {
		//String response = "";
		ArrayList<String> table=new ArrayList<String>();
		table.add(make);
		table.add(model);
		table.add(String.valueOf(baseprice));
		for (int i=0;i<opset.size();i++) {
			table.add(opset.get(i).getName());
			for (int j=0;j<opset.get(i).getOpt().size();j++) {
				table.add(opset.get(i).getOpt().get(j).getName());
				table.add(String.valueOf(opset.get(i).getOpt().get(j).getPrice()));
			}
			table.add("Next");
		}
		return table;
	...
	public String printSTR() {
		String response = "";
		response+=String.format("%s %s starting at a base price of $%.2f.\nSee additional options below:\n", make, model,baseprice);

		for (int i = 0; i < opset.size(); i++) {
			response+=String.format("********************** %-25s **********************\n", opset.get(i).getName());
			response+=opset.get(i).printSTR();
			response+="\n";
		}
		response+=("Total after options: $" + getTotalPrice());
		return response;
	}
