...
public class BuildCarModelOptions implements AutoServer {
	...
	public String setRequest(int request) {
		...
		case 4:
			return("Printing out all autos for the web client");
		default:{
			this.request=-1;
	...
	public ArrayList<Automobile> getAllAutos(){
		//proxyAutomobile.getAutomobileObjects().get(1).getOpset().get(request)
		ArrayList<Automobile> allAutos=new ArrayList<Automobile>();
		for (int i=1;i<=proxyAutomobile.getAutomobileObjects().size();i++)
			allAutos.add(proxyAutomobile.getAutomobileObjects().get(i));
		return allAutos;
	}
	...
	public String processRequest(Object fromClient) throws AutoException {
	 ...
	 public String printAllAutos() {
		 return proxyAutomobile.PrintAllAutos();
	 }
